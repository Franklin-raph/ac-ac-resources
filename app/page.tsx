...
const useNavToggle = () => {
  useEffect(() => {
    const openNav = () => {
      const navList = document.querySelector("nav ul") as HTMLElement | null;
      const menuIcon = document.querySelector(".ri-menu-3-line") as HTMLElement | null;
      const closeIcon = document.querySelector(".ri-close-fill") as HTMLElement | null;
      
      if (navList) navList.style.top = "200px";
    ...
    return () => {
      ...
      if (closeIcon) {
        closeIcon.removeEventListener('click', closeNav);
      }
    };
  }, []);
};
      ...
      <nav className='flex items-center justify-between shadow-sm w-full bg-white z-10 px-[3rem] lg:px-[10rem] py-4'>
        <Link href="/">
          ...
          <div className="togglers hidden text-[20px]">
            <i className="ri-menu-3-line cursor-pointer"></i>
        ...
        <ul className='flex items-center justify-between gap-7 text-[14px] text-gray-600'>
          ...
          <li>
            <Link href='#our-mission-vision'>Mission and Vision</Link>
          </li>
          <li>
      ...
      <div data-aos="fade-up">
                  ...
                  <div>
                    <div className='flex items-center justify-center md:justify-start gap-5'>
                        <button className='bg-primary-color text-white px-5 py-2 rounded-full w-full flex justify-center items-center gap-2'>
                            <i className="ri-apple-line text-[20px]"></i>
                            <p className='hidden sm:block'>Apple Store</p>
                        </button>
                        <button className='bg-primary-color text-white px-5 py-2 rounded-full w-full flex justify-center items-center gap-2'>
                            <i className="ri-google-play-line text-[20px]"></i>
                    ...
                    <button onClick={() => {
                      window.location.href = 'https://go-tru-hub.vercel.app/'
                        ...
                        <i className="ri-external-link-line text-[20px]"></i>
        ...
        </div>
        {
          ...
          <div className='fixed z-10 bottom-3 text-white right-6 bg-primary-color px-1 cursor-pointer rounded' onClick={() => {
            ...
            <i className="ri-arrow-up-s-line text-[24px]"></i>
          </div>
        }
    </main>
  );
